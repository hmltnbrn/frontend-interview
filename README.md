...
- Using React's templating (JSX) requires more work than other frameworks. One example is that you're required to either bind CSS modules or use "className" (in most cases, both). While this isn't that big of a deal for an experienced developer, it might be confusing for a new one. Other frameworks simplify this.
- It isn't completely designed for typed languages (TypeScript or Flow). You can definitely add them now, but vanilla JS is still the default option. In Angular, TypeScript is default.
